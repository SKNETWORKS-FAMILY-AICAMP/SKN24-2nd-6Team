...
[최종 결론 및 비즈니스 활용: Streamlit 기반 타겟팅 솔루션]

본 프로젝트는 분석에 그치지 않고, 모델이 도출한 핵심 변수(기기 사용일수, 가입 기간)를 바탕으로 실제 마케팅 부서에서 활용 가능한 이탈 방지 프로모션 시뮬레이터를 Streamlit으로 구현했습니다.

데이터 기반 타겟팅: 분석 결과, '기기 사용일수'가 길수록 이탈 확률이 급격히 높아지는 경향을 확인했습니다. 이를 근거로 휴대폰 교체 주기에 도달한 장기 사용 고객을 '단말기 지원금' 프로모션의 우선 대상자로 선정했습니다.

- 유연한 의사결정 지원

이탈 확률 임계값(Threshold) 조절: 통신사의 예산 상황에 따라 이탈 위험군을 얼마나 타이트하게 잡을지 사용자가 직접 설정할 수 있습니다.

최소 단말기 사용 일수 필터링: 특정 교체 주기(예: 2년 이상 사용 등)에 도달한 고객군만 필터링하여 프로모션 효율을 극대화합니다.

기대 효과: 통신사는 정해진 마케팅 비용 내에서 이탈 가능성이 가장 높은 고객에게 최적의 혜택을 제공함으로써, 고객 유지 비용(Retention Cost)을 절감하고 서비스 이탈률(Churn Rate)을 효과적으로 낮출 수 있습니다.
